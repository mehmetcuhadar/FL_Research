    ...
    for worker_idx in range(num_workers):
        if worker_idx in poisoned_worker_ids:
            if attack_type == "label_flipping":
                ...
                poisoned_dataset.append(apply_backdoor(distributed_dataset[worker_idx][0], distributed_dataset[worker_idx][1], args.get_target(),args.get_backdoor_intense()))
                ...
                for i in range(len(poisoned_worker_ids)):
                    if poisoned_worker_ids[i] == worker_idx:
                        index = i
                remainder = index % 4
                poisoned_dataset.append(apply_dba(distributed_dataset[worker_idx][0], distributed_dataset[worker_idx][1], args.get_target(), args.get_backdoor_intense(),remainder))
            else:
